...
## Task 9: Searchable Student Selector on Progress Page

**Files:**
- Modify: `app/(dashboard)/admin/milestones/progress/StudentProgressClient.tsx`

**Problem:** Plain `<select>` is hard to use when student list is large. Replace with searchable combobox — type to filter, click to select, X to clear.

- [ ] **Step 1: Add search state and click-outside ref**

In `StudentProgressClient.tsx`, add imports and state:
...
Add helpers after `updateStatus`:
...
- [ ] **Step 2: Replace `<select>` with searchable combobox**

Replace the Student Selector section in the JSX:
  ...
  <div className="relative mt-2" ref={searchRef}>
    {selectedStudent && !showDropdown ? (
      <div className="flex items-center gap-3 rounded-xl border border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/20 px-4 py-3">
        ...
        <button onClick={clearStudent} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
          <X className="h-4 w-4" />
        </button>
      </div>
    ) : (
...
git commit -m "feat: replace student dropdown with searchable combobox on milestone progress page"
```
