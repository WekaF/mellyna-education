# Analytics DataTable Migration Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the three plain HTML tables in the admin analytics page with the existing `DataTable` component (TanStack React Table) to get sorting, pagination, and built-in search.

**Architecture:** `AnalyticsClient.tsx` is a pure client component. The only file to modify is `AnalyticsClient.tsx` — define three `useMemo` column definitions (one per tab), remove the manual `search` state + `filter()` calls, replace the `<table>` blocks with `<DataTable>`. The export buttons keep exporting all data (unfiltered) which is correct for a full report.

**Tech Stack:** Next.js 14 App Router, `@tanstack/react-table` v8, `DataTable` at `components/common/DataTable.tsx`, Tailwind CSS

---

## File Map

- Modify: `app/(dashboard)/admin/analytics/AnalyticsClient.tsx` — replace manual tables with DataTable

No other files need to change.

---

### Task 1: Replace Attendance Tab Table

**Files:**
- Modify: `app/(dashboard)/admin/analytics/AnalyticsClient.tsx`

This is the only task. The three tabs are tightly coupled in one component, so we refactor the whole file at once. The plan breaks it into logical steps.

- [ ] **Step 1: Read and understand current file**

Read `app/(dashboard)/admin/analytics/AnalyticsClient.tsx` in full. Note:
- The `search` state + `filteredAttendance` / `filteredTutors` / `filteredStudents` must be removed
- Three `<table>` blocks (one per tab) must be replaced with `<DataTable>`
- The `<input type="text">` search box must be removed (DataTable has its own built-in)
- Export buttons must stay — they'll export all data from `attendance`, `tutors`, `students` arrays directly (not the filtered subset)
- `RateBar` component must stay

- [ ] **Step 2: Rewrite AnalyticsClient.tsx**

Replace the file content with the following. This is a complete rewrite of `AnalyticsClient.tsx`:
...
```bash
cd /Users/weka/project/mellyna-education && npx tsc --noEmit 2>&1 | grep -E "AnalyticsClient|error"
```

Expected: no output (no errors in AnalyticsClient.tsx)

- [ ] **Step 4: Run dev server and verify**

```bash
cd /Users/weka/project/mellyna-education && npm run dev
```

Open http://localhost:3000/admin/analytics and verify:
- Each tab shows a DataTable with search box and sort arrows on column headers
- Pagination footer shows "Baris per halaman" selector
- Export Excel/PDF buttons still visible above each table
- Switching tabs resets to the correct table

- [ ] **Step 5: Commit**

```bash
git add app/(dashboard)/admin/analytics/AnalyticsClient.tsx
git commit -m "feat: migrate analytics page tables to DataTable component"
```
