...
async function loadLogoResized(filename: string, widthPx: number): Promise<string | null> {
  try {
    const logoPath = path.join(process.cwd(), 'public', 'icons', filename)
    const buf = await sharp(logoPath).resize(widthPx).png().toBuffer()
...
export async function generateInvoicePdf(invoice: InvoiceData): Promise<ArrayBuffer> {
  // Resize to 2× display size for crisp rendering without bloating the PDF
  const logoDark = await loadLogoResized('mellyna-logo-dark.png', 340)
  ...
  // ── Logo (resized PNG, fallback to vector text) ───────────
  if (logoDark) {
    // Display at 170×60 pt — natural ratio of 620:220 ≈ 2.82:1
    doc.addImage(`data:image/png;base64,${logoDark}`, 'PNG', P, 12, 170, 60)
  } else {
